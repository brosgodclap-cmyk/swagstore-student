...
class Account {
  ...
  static canPerformAction(account, action) {
    if (!account) return false;
    
    // Nếu tài khoản là staff, cho phép làm các hành động về sản phẩm và order hộ khách
    ...
    // Nếu là admin thì được làm mọi thứ (giữ nguyên logic mặc định thông thường nếu cần)
    if (account.role === 'admin') return true;

    // Mặc định khách hàng (customer) thông thường không có các quyền trên
    return false;
  }
