...
}); // Đóng khối Account.update


// ĐOẠN TEST MỚI NẰM ĐỘC LẬP Ở ĐÂY:
describe('Account Role & Permissions (Staff Feature)', () => {
  
  test('Tài khoản có role staff PHẢI CÓ QUYỀN CRUD sản phẩm và tạo đơn cho khách', () => {
    const staffUser = {
      id: 123,
      name: "Nhân viên A",
      role: "staff"
    ...
    expect(Account.canPerformAction(staffUser, 'create_product')).toBe(true);
    expect(Account.canPerformAction(staffUser, 'update_product')).toBe(true);
    expect(Account.canPerformAction(staffUser, 'delete_product')).toBe(true);
    expect(Account.canPerformAction(staffUser, 'create_order_for_customer')).toBe(true);
  });

  test('Tài khoản customer thông thường KHÔNG ĐƯỢC có các quyền của staff', () => {
    const customerUser = {
      id: 456,
      name: "Khách hàng B",
      role: "customer"
    ...
    expect(Account.canPerformAction(customerUser, 'create_product')).toBe(false);
    expect(Account.canPerformAction(customerUser, 'delete_product')).toBe(false);
    expect(Account.canPerformAction(customerUser, 'create_order_for_customer')).toBe(false);
  });
});